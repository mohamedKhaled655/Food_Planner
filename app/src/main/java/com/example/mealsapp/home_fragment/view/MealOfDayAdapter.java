...
public class MealOfDayAdapter extends RecyclerView.Adapter<MealOfDayAdapter.MealOfDayViewHolder>{

    private final Context context;
    private List<MealModel> mealModelList;
    private final OnAddFavClickListener listener;

    public MealOfDayAdapter(Context context, List<MealModel> mealModelList, OnAddFavClickListener listener) {
    ...
    @NonNull
    @Override
    public MealOfDayViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.item_random_meal, parent, false);
        return new MealOfDayViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull MealOfDayViewHolder holder, int position) {
        ...
        Glide.with(context).load(imgUrl).placeholder(R.drawable.loadimg)
                .error(R.drawable.ff).into(holder.mealImage);
    ...
    public static class MealOfDayViewHolder extends RecyclerView.ViewHolder {
        TextView txtMealTitle, txtMealId;
        ImageView mealImage;
        ImageButton btnImgFav;
        ImageButton btnCalender;

        public MealOfDayViewHolder(@NonNull View itemView) {
            super(itemView);
            txtMealTitle = itemView.findViewById(R.id.meal_title);
            //txtMealId = itemView.findViewById(R.id.price);
            mealImage = itemView.findViewById(R.id.meal_image);
            btnImgFav = itemView.findViewById(R.id.addtofav);
            btnCalender = itemView.findViewById(R.id.addtoplan);
        }
    }
